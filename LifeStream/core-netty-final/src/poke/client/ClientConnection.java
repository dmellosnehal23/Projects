...
public class ClientConnection {
	...
	public void uplodImage(String userEmail, String name, float latitude, float longitude, String pathname) {
	//	ByteString imageData = this.fileToBytes(pathname);
		
	//	if(imageData == null) return;
		
		// data to send
		ImageInfo.Builder img = ImageInfo.newBuilder();
		img.setUseremail(userEmail);
		img.setName(name);
		img.setLatitude(latitude);
		img.setLongitude(longitude);
		ByteString imageData = ByteString.copyFrom(("Hello " + userEmail).getBytes());
		img.setData(imageData);

		// payload containing data		
		eye.Comm.Payload.Builder p = Payload.newBuilder();
		p.setImage(img.build());
		...
		h.setTag("Add Image");
		h.setTime(System.currentTimeMillis());
		h.setRoutingId(eye.Comm.Header.Routing.IMAGEADD);
	...
	private ByteString fileToBytes(String pathname)
	{
		//meet's code
		byte[] imageInByte;
		ByteString b=null;
		...
		try {
			...
			b=ByteString.copyFrom(imageInByte);
