...
import poke.monitor.ActivityMonitor;
...
public class Server {
	...
	private void init(File cfg) {
		...
		try {
			...
			logger.error("decode expection : " + e.getMessage());
		}
	...
	protected void run() {
		...
		//SNE: start activity monitor
		activityMonitor = ActivityMonitor.getInstance();
		activityMonitor.initialize(conf);
