...
import poke.server.routing.Router;
...
public class PerChannelQueue implements ChannelQueue {
	...
	protected static HashMap<Integer, Bootstrap> routeBootstrap = new HashMap<Integer, Bootstrap>();
	...
	protected void init() {
		...
		//initialize router to route requests
		this.router = new Router(this.tgroup, this);
	}
				...
				try {
					...
					if (conn.isWritable()) {
						...
						if (channel != null && channel.isOpen()
								&& channel.isWritable()) {
							//logger.info("OUT CHANNEL INFO " + channel.getLocalAddress().toString());
			...
			while (true) {
				...
				try {
					...
					if (msg instanceof Request) {
						...
						//logger.info("IN CHANNEL INFO " + channel.getLocalAddress().toString());
						//logger.info(msg.toString());
						Response reply = null;
						if (rsc == null) {
