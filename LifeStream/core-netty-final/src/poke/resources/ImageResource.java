...
import gash.jpa.ejb.JMSRequestor;
...
public class ImageResource implements Resource {
	...
	public Response process(Request request) {
		...
		Response reply = null;
		try {
			logger.info("Img req : " + request.getHeader().getTag());
			JMSRequestor requestor = new JMSRequestor();
			requestor.uploadImage(request.getBody().getImage().getUseremail(), 
							request.getBody().getImage().getLatitude(),
							request.getBody().getImage().getLongitude(),
							request.getBody().getImage().getData().toByteArray());
